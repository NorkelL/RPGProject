...
- **Pull Request (PR)** (Vorschlag, einen Branch in einen anderen zu mergen, wird auf GitHub diskutiert und reviewed)
...
1. **Aufgabe holen:** Im Project-Board `work` oder im Issues-Tab ein offenes Issue suchen. **Sich selbst als Assignee eintragen**, sonst arbeiten am Ende zwei Leute am selben Ding. Issue auf dem Board nach `In Progress` schieben.
...
- `*.ctxt`-Dateien (BlueJ/Greenfoot Kontext): committen, aber Diff-Lärm ist normal
- `project.greenfoot`, `project.properties`: werden bei jedem Öffnen verändert, häufige Konfliktquelle
- `.class`-Dateien: gehören in `.gitignore`, **nie committen**
- Bilder/Sounds in `images/` und `sounds/`: Binärdateien, die nicht mergen. Nur eine Person sollte gleichzeitig daran arbeiten.

Bei Konflikten in Greenfoot-Konfigdateien ist es meist sicherer, eine Version komplett zu übernehmen, statt zeilenweise zu mergen. **Solche Konflikte fallen unter Regel 5, also zu Nikolaj oder Tom.**
