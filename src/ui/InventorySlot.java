...
public class InventorySlot extends Actor {
    private Actor item;
    private GreenfootImage baseImage; // Initialize in constructors

    // Default constructor, calls the parameterized one with a default size
    ...
    public InventorySlot(Actor item) {
        this(); // Calls the default constructor, which in turn calls the parameterized one
        setItem(item);
    }
